 ...
 *
 * 它代表了一组ChannelHandler 用来处理或者一个Channel的拦截输入事件以及输出操作 ...
 * ChannelPipeline 实现了一种高级的形式(拦截过滤器模式),让用户能够完全控制一个事件应该如何处理并且在pipeline中的ChannelHandler 相互交互 ...
 *
 *
 * 1. pipeline 的创建
 *
 * 每一个channel 都包含一个自己的pipelineg 并且它们能够在新的channel 创建时自动的创建 ...
 * 2. pipeline中的事件流处理
 *  以下的表说明了如何通过ChannelPipeline中的ChannelHandler 处理 IO 事件 ...
 *  一个IO 事件要么通过 ChannelInboundHandler 或者ChannelOutboundHandler 进行处理并向后转发它最贴近的一个handler(通过调用在ChannelHandlerContext中
 *  定义的事件传播方法),例如 ChannelHandlerContext#fireChannelRead(Object) 以及 ChannelHandlerContext#write(Object) ..
 *
 ...
 * 下面的图:
 *
 *  一个IO 请求要么从Channel 开始,或者通过ChannelHandlerContext 传递的
 *
 * <pre>
 *                                                 I/O Request
 *                                            via {@link Channel} or
 ...
 * </pre>
 *
 * 一个输入事件是通过在图中左边所展示的一个自下而上的方式处理 ..
 * 一个输入处理器 通常给处理的输入数据通常是从图中最底下的 I/O 线程所生成的 ..
 * 那么这样的一个输入数据通常是从一个对等远方读取的(通过实际的输入操作,例如 SocketChannel#read(ByteBuffer)进行读取)
 * 如果输入事件超出最顶部输入处理器之外,那么它将安静的抛弃,或者记录(取决于你的关注点) ...
 ...
 *
 *
 * 一个输出事件是通过由上而下的方向被输出处理器处理 ...(它位于图的右边) ...
 * 一个outbound handler 通常生成或者转换一个输出流量(例如 写请求) ..
 * 如果一个输出事件超出最底下的输出处理器之外(经过),他将被一个 I/0 线程所处理(与这个Channel 关联的Thread) ...
 * 这个I/0 线程通常执行实际的输出操作(例如 SocketChannel#write(ByteBuffer)) ...
 * <p>
 ...
 * </pre>
 *
 * 上面的例子中 以Inbound 标识它是一个inbound handler ..
 * 类以Outbound 开头的名字意味着它是一个输出handler ...
 ...
 * <p>
 *     在给定示例配置中, 处理器评估顺序是  1 , 2 , 3, 4, 5(当事件输入时)
 *     当一个事件是outbound, 顺序是 5,4,3,2,1, 在这个原理之上,ChannelPipeline 将会跳过某些处理器的评估(去缩短调用栈深度):
 *     1. 3 和 4 没有实现ChannelInboundHandler,  仅仅只有实际的inbound event handler will evaluation ...
 *     等价于 1  ,2 , 5
 *
 *     同理,outbound的情况下,仅仅只有 5,4,3 将会被处理...
 *     如果5同时实现了ChannelInboundHandler (对于inbound / outbound 事件单独是 125 / 543)
 ...
 *
 * 你可能注意到上图展示中,一个handler可以执行ChannelHandlerContext的事件传播方法去转发一个事件到下一个handler中, 这些方法包括:
 *
 * As you might noticed in the diagram shows, a handler has to invoke the event propagation methods in
 * {@link ChannelHandlerContext} to forward an event to its next handler.  Those methods include:
 * <ul>
 *     输入数据传播方法:
 *     也就是说pipeline中的handler 是串行执行 ...(如果不想触发下一个handler,就可以直接返回) ...
 ...
 *     // 也就是 read动作可以切换 outbound 到 inbound中 处理
 ...
 *
 * 一个用户假设包含一个或者多个ChannelHandler (在一个pipeline中) 接受IO 事件(例如: 读取),并请求IO 操作(例如 写或者关闭) ..
 * 请注意这两种观念 ...
 * 例如,一个服务器通常包含以下的处理器到每一个管道的pipeline中,但是你handler 可能特别依赖与协议以及业务逻辑的复杂性和特征 ...
 * <p>
 ...
 *
 * 如果业务逻辑是完全异步的,或者完成的非常快速,不需要指定一个group ...
 * pipeline.addLast(group, "handler", new MyBusinessLogicHandler());
 * </pre>
 * 注意到 DefaultEventLoopGroup 将会卸载来自 EventLoop的操作(将在一种连续的方式(串行)处理任务(根据每一个ChannelHandlerContext,仍然能够保证顺序) ..
 * 由于这仍然是一种瓶颈 ...
 * 使用无顺序的 ... 事件执行器 最大化任务执行的并行度 ...
 ...
 * after the exchange.
 *
 * 一个ChannelHandler 能够增加或者移除(在任何时刻),因为ChannelPipeline是线程安全的 ..
 * 举个例子,你能够插入一个加入处理器(当敏感信息发生了交换时,并在交换之后移除它) ....
 */
